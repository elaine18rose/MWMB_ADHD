...
stdRT_ADHD_inn=[];
for nc=1:length(adhds_inn)
    stdRT_ADHD_inn(nc)=nanmean(all_behav_table.stdRT(all_behav_table.SubID==adhds_inn(nc)));
end
stdRT_ADHD_com=[];
for nc=1:length(adhds_com)
    stdRT_ADHD_com(nc)=nanmean(all_behav_table.stdRT(all_behav_table.SubID==adhds_com(nc)));
end
...
%% Intentionality  (Note: 1 = Entirely intentional to 4 = Entirely unintentional)
  numbers_of_interest = [1, 2, 3, 4];
    ...
    for nc = 1:length(ctrs)
        ...
        % Extract the State column for the current participant
        int_values = probe_subtype_table.Intention(probe_subtype_table.SubID == ctrs(nc));
        % Count occurrences of each number (1, 2, 3, 4) for the current participant
    ...
    for nc = 1:length(adhds_inn)
        % Extract the State column for the current participant
        int_values = probe_subtype_table.Intention(probe_subtype_table.SubID == adhds_inn(nc));
        % Count occurrences of each number (1, 2, 3, 4) for the current participant
    ...
    for nc = 1:length(adhds_com)
        % Extract the State column for the current participant
        int_values = probe_subtype_table.Distraction(probe_subtype_table.SubID == adhds_com(nc));
        % Count occurrences of each number (1, 2, 3, 4) for the current participant
...
mdlstdRT0  = fitlme(behav_demo_table1,'stdRT~1+BlockN+ReportedSubtype+(1|SubID)'); % Winning model
% mdlstdRT1  = fitlme(behav_demo_table1,'stdRT~1+BlockN*ReportedSubtype+(1|SubID)'); 
% mdlstdRT2  = fitlme(behav_demo_table1,'stdRT~1+BlockN*ReportedSubtype+(BlockN|SubID)'); 
% %%% Extract fit statistics for each model
