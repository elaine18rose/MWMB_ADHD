...
%% 24/10/25 adding age as a covariate to address reviewer comments
clear all;
close all;
%%
...
subject_age(subject_age.SubID == 'C015', :) = [];
...
SW_table = join(SW_table, subject_age, 'Keys', 'SubID'); % adding age 
...
% Edited 24/10/25 to address reviewer comments - adding age as a covariate
SW_table.Group=categorical(SW_table.Group);
...
for nE=1:length(layout.label)-2
    ...
    temp_mdl_byG=fitlme(sub_table,sprintf('SW_density~1+Block*%s+Age+(1|SubID)','Group')); % added age 
...
set(gca,'FontSize',22,'FontWeight','bold','LineWidth', 1.5);
 ...
 %% 24/10/25 Seeing if adding age improves model fit
mdlSWdens = fitlme(SW_table,'SW_density ~ 1+Block+Group+(1|SubID)');
mdlSWdens1 = fitlme(SW_table,'SW_density ~ 1+Block*Group+(1|SubID)');
compare(mdlSWdens, mdlSWdens1)
anova(mdlSWdens1)
mdlSWdens1Age = fitlme(SW_table,'SW_density ~ 1+Block*Group+Age+(1|SubID)');
compare(mdlSWdens1, mdlSWdens1Age) % winning model is mdlSWdens1Age
mdlSWdens1AgeInt = fitlme(SW_table,'SW_density ~ 1+Block*Group+Group*Age+(1|SubID)');
compare(mdlSWdens1Age, mdlSWdens1AgeInt) % winning model is mdlSWdens1Age
anova(mdlSWdens1Age)

%% SW on behav (w/ age as covariate 24/10/25)
% clear topo_*
% clear sub_table
% clear temp_mdl_*
SW_table.Group=categorical(SW_table.Group);
...
for nV=1:length(VOI)
    ...
    for nE=1:length(layout.label)-2
        fprintf('... %g/%g\n',nE,length(layout.label)-2)
        sub_table=SW_table(SW_table.Elec==layout.label(nE),:);
        temp_mdl1=fitlme(sub_table,sprintf('%s~1+Block+SW_density+Age+(1|SubID)',VOI{nV})); % added age 24/10/25
...
%% Mediation (residuals) (w/ age as covariate 24/10/25)
for nV=1:length(VOI)
    topo_med_tV{nV}=[];
    topo_med_pV{nV}=[];
    for nE=1:length(layout.label)-2
        fprintf('... %g/%g\n',nE,length(layout.label)-2)
        sub_table=SW_table(SW_table.Elec==layout.label(nE),:);
        temp_mdl1=fitlme(sub_table,sprintf('%s~1+Block*Group+Age+(1|SubID)',VOI{nV}));
        res_mdl1=residuals(temp_mdl1);

        temp_mdl2=fitlme(sub_table,'SW_density~1+Block*Group+Age+(1|SubID)');
        ...
        temp_mdl4=fitlme(sub_table,sprintf('%s~1+Block+SW_density+Age+(1|SubID)',VOI{nV}));
...
% Save figure
 saveas(gcf, [pwd filesep 'Figures' filesep 'Fig5_PanelA_SWxBehav_withage.svg']);
...
% Save figure
 saveas(gcf, [pwd filesep 'Figures' filesep 'Fig5_PanelC_MedSWxBehav_withage.svg']);
...
for nCh=1:length(layout.label)-2
    ...
    if nCh==1
        out_pred_perm=[];
        [real_out, cont_out, perm_out, cont_perm_out, out_pred_perm]=lme_perm_bygroup_interaction(sub_table,'Group','SW_density~1+Block*pred+Age+(1|SubID)',totperm); % added age as a covariate 24/10/25
    else
        [real_out, cont_out, perm_out, cont_perm_out, next_out_pred_perm]=lme_perm_bygroup_interaction(sub_table,'Group','SW_density~1+Block*pred+Age+(1|SubID)',totperm,out_pred_perm); % added age as a covariate 24/10/25
    end
...
%% Topos of the relationship between Group and SWDens using LMEs and cluster permutation (with age as a covariate 24/10/25)
clus_alpha=0.05;
...
lgd = legend([h1, h2], {'Uncorrected p < .05', 'Cluster Permutation p < .05'}, 'Box', 'off');
lgd.Position = [0.16, 0.05, 0.1, 0.1]; % horizontal,vertical, width, height 
lgd.FontSize = 28;

% sgtitle(['Slow Wave Density'],'FontWeight', 'bold', 'FontSize', 50);
% % Save figure
 saveas(gcf, [pwd filesep 'Figures' filesep 'Fig4_PanelC_SWxGroup_withage.svg']);


 %% 14/11/25 Addressing reviewer comments about SW frequency 
 freq_bins = 1:0.5:7;
 bin_centers = freq_bins(1:end-1) + diff(freq_bins)/2;

subs = unique(SW_table.SubID);
all_freq_CTRL = [];
all_freq_ADHD = [];
...
for s = 1:length(subs)
    ...
    subID = subs(s);
    sub_data = SW_table(SW_table.SubID == subID, :);
    group = unique(sub_data.Group);

    % Histogram of SW frequencies for this participant
    [counts, ~] = histcounts(sub_data.SW_frequency, freq_bins);

    counts = counts / sum(counts);  

    if group=='Control'
        all_freq_CTRL = [all_freq_CTRL; counts];
    elseif group=='ADHD'
        all_freq_ADHD = [all_freq_ADHD; counts];
    end
end


%% --- Plot Panel A: Group distributions ---
figure('Position',[350 300 450 320]);
hold on;
...
[~, h(1)] = simpleTplot(bin_centers, all_freq_CTRL, 0, Colors(1,:), [0], '-', 0.5, 1, 0, 1, 4);
[~, h(2)] = simpleTplot(bin_centers, all_freq_ADHD, 0, Colors(2,:), [0], '-', 0.5, 1, 0, 1, 4);

legend(h, {'Neurotypical','ADHD'}, 'Box','off');
xlabel('SW Frequency (Hz)');
ylabel('Normalised SW Distribution');
xlim([freq_bins(1) freq_bins(end)]);
format_fig;

% saveas(gcf, [pwd filesep 'Figures' filesep 'Fig_SW_FreqDist.svg']);


%% --- Plot Panel B: ADHD - Control difference ---
mean_diff = nanmean(all_freq_ADHD) - nanmean(all_freq_CTRL);

figure('Position',[350 300 450 320]);
hold on;

simpleTplot(bin_centers, all_freq_ADHD - nanmean(all_freq_CTRL), ...
            0, Colors(2,:), [0 0.05 0.05 1000], '-', 0.5, 1, 0, 1, 4);

yline(0, 'k', 'LineWidth', 2);
xlabel('SW Frequency (Hz)');
ylabel({'Density Diff (ADHD – NT)', 'proportion'});
xlim([freq_bins(1) freq_bins(end)]);
format_fig;

%% LME for SW Frequency
...
for s = 1:length(subs)
    ...
    subID = subs(s);
    sub_data = SW_table(SW_table.SubID == subID, :);
    group = string(unique(sub_data.Group));

    [counts, ~] = histcounts(sub_data.SW_frequency, freq_bins);
    counts = counts / sum(counts);

    for b = 1:length(bin_centers)
        SWFreqLME_data(row).SubID   = string(subID);
        SWFreqLME_data(row).Group   = group;
        SWFreqLME_data(row).Bin     = bin_centers(b);
        SWFreqLME_data(row).Prop    = counts(b);
        row = row + 1;
    end
end

SWFreqLME_data = struct2table(SWFreqLME_data);

SWFreqLME_data.SubID = categorical(SWFreqLME_data.SubID);
SWFreqLME_data.Group = categorical(SWFreqLME_data.Group);

mdl = fitlme(SWFreqLME_data, ...
    'Prop ~ Group*Bin + (1|SubID)');
