...
badTrials_all = badChannels_badTrials_info(:,7);
total_removed_probes = sum(cellfun(@length, badTrials_all));
fprintf('Total number of removed probes: %d\n', total_removed_probes);
badTrials_all_numeric = cellfun(@numel, badTrials_all);
meanbadTr = mean(badTrials_all_numeric);
stdbadTr = std(badTrials_all_numeric);
SEMbadTr = stdbadTr / sqrt(length(badTrials_all_numeric));
fprintf('Average number of bad probes: %.2f ± %.2f\n', meanbadTr, SEMbadTr);


numInterpolated = cellfun(@length, badChannels_badTrials_info(:, 4)); % Column 4 stores badChannels
total_int_channels = sum(numInterpolated);
fprintf('Total number of interpolated channels: %d\n', total_int_channels);
meanInterpolated = mean(numInterpolated);
stdInterpolated = std(numInterpolated);
SEMInterpolated = stdInterpolated / sqrt(length(numInterpolated));
fprintf('Average number of interpolated channels: %.2f ± %.2f\n', meanInterpolated, SEMInterpolated);
