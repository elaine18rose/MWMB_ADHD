%% version 2 - puts NA values for no waves found for an electrode or if waves detected are below 50  
clear all;
close all;
%%
...
for nF=1:length(files)
    ...
    if redo==1 || exist([preproc_path filesep 'SW_clean_i_probe_' SubID '.mat'])==0 % To skip already preprocessed files
        ...
        for nE=1:length(labels)
            ...
            % The following 5 lines are copied from CTET_ADHD2_analysse_selectLocalSleep_exGaussCTRThr_v4.m script
            % as it was crashing for C038 due to different sizes (Error
            % using fminsearchbnd x0 is incompatible in size with either LB
            % or UB.). It did this for Elec O2 and and PO7
            if length(temp_p2p)<50 % was 100 but causing an error for A039 because it had less than 100 waves across all electrodes
                warning(sprintf('not enough waves (<50) to compute a threshold!!! Sub %s Elec %s\n',SubID,labels{nE}))
                thr_Wave_eg(nFc,nE)=NaN;   % If the amount of P2P waves detected is less than 50, NaN values is saved in thr_Wave which saves the threshold for each electrode per participant
                thr_Wave_pc(nE)=prctile(thisE_Waves(:,paramSW.AmpCriterionIdx),paramSW.prticle_Thr); % Getting percentile threshold
                continue;
            end
            ...
            thr_Wave_eg(nE)=end_gaussian;      % Otherwise, if paramS W.fixThr is empty, save the value obtained in end_gaussian (.fixThr should be empty)
        end
...
writetable(all_threshold_SW,[preproc_path filesep 'all_threshold_SW_90pc.csv'])
...
exclude_ids = {'C017', 'C038'};
exclude = ~ismember(all_threshold_SW.SubID, exclude_ids);
figure;
simpleCorPlot(all_threshold_SW.Thr_PC(exclude), all_threshold_SW.Thr_EG(exclude));
xlabel('Percentile threshold');
ylabel('Ex-Gaussian threshold');
title('Without C017 and C038');
