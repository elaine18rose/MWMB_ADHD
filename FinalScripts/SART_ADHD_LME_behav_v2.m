...
all_probe_table.Group = categorical(all_probe_table.Group); % Convert to categorical
...
mdl_RTxState = fitlme(all_probe_table, 'RT ~ 1 + Block + Group*StateC  + (1|SubID)', 'DummyVarCoding', 'reference');
...
subtypes = unique(agg_data_ADHD.DIVASubtype); % 
comorbidity_stats = zeros(numel(subtypes), 2); % Initialize a matrix for stats

for i = 1:numel(subtypes)
    idx = agg_data_ADHD.DIVASubtype == subtypes(i); % Logical index for each subtype
...
% Seeing if Depression and Anxiety have an effect on performance 
% FAs/Commission Errors
behav_demo_table.Depression = categorical(behav_demo_table.Depression);
mdldepFA0  = fitglme(behav_demo_table,'FA~1+BlockN+Depression+(1|SubID)');
mdldepFA1  = fitglme(behav_demo_table,'FA~1+BlockN*Depression+(1|SubID)');
compare(mdldepFA0, mdldepFA1)
anova(mdldepFA1) % Block and Block*Depression

behav_demo_table.Anxiety = categorical(behav_demo_table.Anxiety);
mdlanxFA0  = fitglme(behav_demo_table,'FA~1+BlockN+Anxiety+(1|SubID)');
mdlanxFA1  = fitglme(behav_demo_table,'FA~1+BlockN*Anxiety+(1|SubID)');
compare(mdlanxFA0, mdlanxFA1)
anova(mdlanxFA1) % Block and Block*Anxiety


% Misses/Omission Errors
mdldepMiss0  = fitglme(behav_demo_table,'Misses~1+BlockN+Depression+(1|SubID)');
mdldepMiss1  = fitglme(behav_demo_table,'Misses~1+BlockN*Depression+(1|SubID)');
compare(mdldepMiss0, mdldepMiss1)
anova(mdldepMiss1) % Block and Block*Depression

mdlanxMiss0  = fitglme(behav_demo_table,'Misses~1+BlockN+Anxiety+(1|SubID)');
mdlanxMiss1  = fitglme(behav_demo_table,'Misses~1+BlockN*Anxiety+(1|SubID)');
compare(mdlanxMiss0, mdlanxMiss1)
anova(mdlanxMiss1) % Block and Block*Anxiety


%RT
mdldepRT0  = fitlme(behav_demo_table,'RT~1+BlockN+Depression+(1|SubID)');
mdldepRT1  = fitlme(behav_demo_table,'RT~1+BlockN*Depression+(1|SubID)');
compare(mdldepRT0, mdldepRT1)
anova(mdldepRT1) % Block and Block*Depression

mdlanxRT0  = fitlme(behav_demo_table,'RT~1+BlockN+Anxiety+(1|SubID)');
mdlanxRT1  = fitlme(behav_demo_table,'RT~1+BlockN*Anxiety+(1|SubID)');
compare(mdlanxRT0, mdlanxRT1)
anova(mdlanxRT1) % Block and Block*Anxiety


% Standard Deviation of Reaction Times
behav_demo_summary = varfun(@(x) x(1), behav_demo_table, ...
    'GroupingVariables', 'SubID', ...
    'InputVariables', {'Depression', 'Anxiety'}); %Summary table because there were SubID repeats as it was byTrial
behav_demo_summary.Properties.VariableNames{'Fun_Depression'} = 'Depression';
behav_demo_summary.Properties.VariableNames{'Fun_Anxiety'} = 'Anxiety';
behav_demo_summary.SubID = categorical(behav_demo_summary.SubID);
behav_demo_summary.Depression = categorical(behav_demo_summary.Depression);
behav_demo_summary.Anxiety = categorical(behav_demo_summary.Anxiety);
block_DepAnx_table = join(all_block_table, behav_demo_summary(:, {'SubID', 'Anxiety', 'Depression'}), 'Keys', 'SubID');
block_DepAnx_table.SubID = categorical(block_DepAnx_table.SubID);
block_DepAnx_table.Group = categorical(block_DepAnx_table.Group);
block_DepAnx_table.Anxiety = categorical(block_DepAnx_table.Anxiety);
block_DepAnx_table.Depression = categorical(block_DepAnx_table.Depression);

mdldepStdRT0 = fitlme(block_DepAnx_table, 'stdRT ~ 1 + Depression + BlockN +(1|SubID)');
mdldepStdRT1 = fitlme(block_DepAnx_table, 'stdRT ~ 1 + Depression * BlockN +(1|SubID)');
compare(mdldepStdRT0, mdldepStdRT1)
anova(mdldepStdRT0)

mdlanxStdRT0 = fitlme(block_DepAnx_table, 'stdRT ~ 1 + Anxiety + BlockN +(1|SubID)');
mdlanxStdRT1 = fitlme(block_DepAnx_table, 'stdRT ~ 1 + Anxiety * BlockN +(1|SubID)');
compare(mdlanxStdRT0, mdlanxStdRT1)
anova(mdlanxStdRT0)
