...
```{r}
# 04/04/25 EP: Quick check to see if I'm getting consistent results with MATLAB function
all_behav_table <- read.table('/Users/elaine/Desktop/Git_Studies/MWMB_ADHD/tables/MWMB_ADHD_all_behav_byTrial.txt', header = TRUE,sep = ",", dec = ".")
lm.Miss0 <- lmer(Misses ~ 1 + BlockN + Group + (1|SubID), data = dataset, REML = FALSE)
lm.Miss1 <- lmer(Misses ~ 1 + BlockN * Group + (1|SubID), data = dataset, REML = FALSE)

anova(lm.Miss0,lm.Miss1)
anova(lm.Miss1, ddf = "Satterthwaite")
summary(lm.Miss1)
...
##TA - One approach is to do an ANOVA and see if there's a main effect of group and then do a post-hoc 
## For those combined it would be a 1 for Combined and 1 for inattentive
