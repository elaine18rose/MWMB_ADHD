...
# Here we're excluding control participants who score above 60 as well as those flagged with havig inconsistent CAARS scores
# dataset2 <- dataset[is.na(dataset$CAARS_ADHDIndex)==FALSE,]
CAARSInattentivenessTH <- dataset %>% 
  filter((Group == "Control" & CAARS_Inattentiveness <61 & CAARS_InconsistencyIndex==0))# | (Group == "ADHD" & CAARS_InconsistencyIndex==0)))
CAARSHyperactivityTH <-  dataset %>% filter((Group == "Control" & CAARS_Hyperactivity <61) & CAARS_InconsistencyIndex==0 ) #| (Group == "ADHD" & CAARS_InconsistencyIndex==0))
CAARSTotalADHDSymptomsTH <- dataset %>% filter((Group == "Control" & CAARS_TotalADHDSymptoms <61) | (Group == "ADHD" ))#& CAARS_InconsistencyIndex==0))
CAARSADHDIndexTH <- dataset %>% filter((Group == "Control" & CAARS_ADHDIndex <61) |  (Group == "ADHD"))#& CAARS_InconsistencyIndex==0))
...
```{r}
#Boxplot for CAARS ADHD Index and Total ADHD symptoms ----
...
ggplot(dataset2, aes(x = Group, y = CAARS_TotalADHDSymptoms)) + 
  geom_boxplot(aes(fill = Group), fill= Colors) +
    geom_jitter(shape = 20, position = position_jitter(width = 0.21), size = 5) +
...
```
```{r}
# testing normality for ttest ----
# Separate data by group
adhd_group <- dataset %>% filter(Group == "ADHD")
adhd_group <- adhd_group %>% 
  group_by(SubID) %>%
  summarise(CAARSADHDIndex = mean(CAARS_ADHDIndex),
            CAARSTotalADHDSymptoms = mean(CAARS_TotalADHDSymptoms))
control_group <- dataset %>% filter(Group == "Control")
control_group <- control_group %>%
    group_by(SubID) %>%
  summarise(CAARSADHDIndex = mean(CAARS_ADHDIndex),
            CAARSTotalADHDSymptoms = mean(CAARS_TotalADHDSymptoms))
# Test normality 
shapiro.test(adhd_group$CAARSTotalADHDSymptoms) 
shapiro.test(adhd_group$CAARSADHDIndex) 
shapiro.test(control_group$CAARSTotalADHDSymptoms) 
shapiro.test(control_group$CAARSADHDIndex) 

# ---- t-test CAARS scores and group ----
t.test(CAARS_ADHDIndex ~ Group, data = dataset)
# t.test(CAARSInattentiveness ~ Group, data = CAARSInattentiveness)
# t.test(CAARSHyperactivity ~ Group, data = CAARSHyperactivity)
t.test(CAARS_TotalADHDSymptoms ~ Group, data = dataset)
