...
ylim([0 0.05]*100)
 set(gca, 'xtick',[-0.1 0.1],'xticklabel',{'C','A'}); %to change y-axis to percentage
    title(['Omission']);
%     ylabel('% of Omission Errors'); xlabel('Block Number');
    format_fig;
    set(gca,'FontSize',20,'FontWeight','bold','LineWidth', 1.5);
...
subjects = [];
 ...
 for i = 1:4 % number of repetitions
    ...
    set(gca, 'xtick',1:4); %to change y-axis to percentage
    title(['Reaction Time across Blocks']);
...
subjects = [];
 ...
 for i = 1:4 % number of repetitions
    ...
    set(gca, 'xtick',1:4); %to change y-axis to percentage
    title(['Std Deviation of RT across Blocks']);
