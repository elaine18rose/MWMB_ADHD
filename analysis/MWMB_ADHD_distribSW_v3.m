...
for nF=1:length(SW_files)
    ...
    all_Waves(all_onset<-20 | all_onset>0 | all_freq>paramSW.max_Freq | all_Waves(:,paramSW.AmpCriterionIdx)>paramSW.art_ampl | all_Waves(:,11)>paramSW.max_posampl| all_Waves(:,14)>paramSW.art_ampl| abs(all_Waves(:,15))>paramSW.art_ampl,:)=[];
     ...
     % SW ampl
        ...
        % SW freq
        bins_freq=1:0.05:7;
        this_freq=1./(abs((all_Waves(:,5)-all_Waves(:,7)))./fsample);
        freq_bins=histc(this_amp,bins_freq);
        freq_bins=(freq_bins/duration_of_recording)/length(labels)';

        all_distrib_freq_SW=[all_distrib_freq_SW ; freq_bins'];
        temp_distr_freq_SW=[];
        for nE=1:length(labels)
            this_freq=1./(abs((all_Waves(all_Waves(:,3)==nE,5)-all_Waves(all_Waves(:,3)==nE,7)))./fsample);
            freq_bins=histc(this_freq,bins_freq);
            freq_bins=(freq_bins/duration_of_recording)';
            temp_distr_freq_SW=[temp_distr_freq_SW ; freq_bins];
        end
                all_distrib_freq_SW_per=cat(3,all_distrib_freq_SW_per, temp_distr_freq_SW);
end
...
hplot=[];
for nG=1:length(uniqueGroupIDs)
    temp_plot=all_distrib_freq_SW(all_GroupID==uniqueGroupIDs(nG),:);
    [pV hplot(nG)]=simpleTplot(bins_freq,temp_plot,0,Colors(nG,:),[0],'-',0.5,1,0,1,4);
end
legend(hplot,uniqueGroupIDs, 'Box', 'off')
format_fig;
xlabel('Frequency all Waves')
ylabel('Slow Wave Density')
xlim([1 7])
% Save figure
% saveas(gcf, [pwd filesep 'Figures' filesep 'Fig4_PanelAi_Dist.svg']);
...
hplot=[];
% for nG=1:length(uniqueGroupIDs)
    temp_plotA=all_distrib_freq_SW(all_GroupID==uniqueGroupIDs(2),:);
     temp_plotB=all_distrib_freq_SW(all_GroupID==uniqueGroupIDs(1),:);
     temp_plotA=temp_plotA-nanmean(temp_plotB);
   simpleTplot(bins_freq,temp_plotA,0,Colors(2,:),[0 0.05 0.05 1000],'-',0.5,1,0,1,4);
% end
% legend(hplot,uniqueGroupIDs)
yline(0, 'k', 'LineWidth', 2);
format_fig;
