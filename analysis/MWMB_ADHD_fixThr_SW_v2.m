...
RS = ["R1", "R2"];
%redo=0;
all_threshold_SW=readtable([preproc_path filesep 'all_threshold_SW.csv']); %EP - This is 
...
%     paramSW.prticle_Thr=90; % 80 or 90 or 95
    ...
    for nBl=unique(slow_Waves(:,2))'
        ...
        for nE=1:length(labels)
            this_thr=5; %av_CTR_threshold_SW.mean_Thr(av_CTR_threshold_SW.Elec==labels{nE});
...
for nE=1:length(layout.label)-2
    ...
    temp_mdl_byG=fitlme(sub_table,sprintf('SW_amplitude~1+Block+%s+(1|SubID)','Group')); 
